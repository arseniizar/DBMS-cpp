...
void Parser::step_foreign_key_as_type() {
    auto foreign = Parser::peek();
    str_toupper(foreign);
    if (foreign != "FOREIGN") {
        Parser::step = Step::error;
        Parser::error_message = "expected a keyword FOREIGN after the type of the Field";
        return;
    }
    Parser::pop();
    ...
    if (key != "KEY") {
        Parser::step = Step::error;
        Parser::error_message = "expected a keyword KEY after FOREIGN";
        return;
    }
    Parser::pop();
